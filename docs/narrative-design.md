...
### 2.5 规则怪谈生成元矩阵 · 来源 × 对象 🔒 LOCKED (v0.5 / 2026-05-05)

#### 2.5.1 双轴 + 核心原理

**核心原理**：规则怪谈**不是**"互相矛盾的多份文档"——是**同一个事实**在不同 (writer source × target audience) 组合下产生的不同表述。多份文档的差异是这个生成元的**症候**，不是原因。

| 轴 | 定义 | 取值数 |
|---|---|---|
| **来源 (sources)** | 写者所属的部门 / 信息流来源 | **9** = 7 正式部门 + 2 非正式来源 |
| **对象 (audiences)** | 文本预期被谁读 | **5** = 5 工种阶梯 |
| **Cell** | 该来源对此对象的具体写作动机 | 9 × 5 = 45 cells |

> **多声部反身闭合（D8）兑现**：规则怪谈的多声部矛盾不是因为信息差/谎言，而是**每份文档的写者处在身份阶梯的不同位置**——读者（玩家）比对矛盾的过程，本质上是在测量每个写者距离深渊有多远。**且玩家自己就是其中一个写者。**

#### 2.5.2 9 来源（7 正式部门 + 2 非正式）

```
X Company
├─ M1 人事与排班办公室          (招募 / 排班 / 调岗 / 特殊勤务包装)
├─ M2 外部文本回收科            (来源遮蔽 / 脱语境化 / 词包化 / R 标签)
├─ M3 文书实验部                (第七打字室 / 任务发放 / 主管系统)
├─ M4 安全部门 (含 MOKO)        (动作流程 / 猴面具 / 非人类输入模型组)
├─ M5 档案与归属办公室          (档案 + 责任归属 + 签名管理)
├─ M6 状态确认组                (离岗测试 / 延长观察 / 猴子规则触发)
└─ M7 猴群管理室                (自由打字区 / 无署名输入源)

非正式来源：
├─ M8 前员工残留                (桌底便签 / 面具内侧划痕 / 废纸批注)
└─ M9 红领结文本                (赞美 / 修改邀请 / 第二人称署名陷阱)
```
...
| 来源 | 录入员 | 校对者 | 修改者 | 作者化个体 | 深度作者化 / 猴子化对象 |
|---|---|---|---|---|---|
| **M1 人事与排班** | 招募、排班、安抚异常反应，把岗位包装成普通工作 | 用"岗位拓展"包装判断权出现 | 用"高级修订岗"包装隔离 | 用"特殊勤务"包装失去离岗资格 | 用"长期输入安排"包装无害化 |
| **M2 外部文本回收** | 提供脱语境、词包化、编号化任务材料 | 提供差异样本与来源遮蔽对照 | 提供受控替换材料 | 限制其接触新原始样本 | 回收其输出，重新投入未受理文本池 |
| **M3 文书实验部** | 让其按时、准确、无解释地完成录入 | 利用其识别错误，但不允许解释 | 利用其改写能力做受控实验 | 利用其生成规则，同时准备移交 | 将其从员工名单转为输入源 |
| **M4 安全部门 (MOKO)** | 用动作流程、MOKO、猴面具维持字符层输入 | 限制判断权，防止识别变成解释 | 控制修改行为，防止个人意图进入文本 | 尝试降级、冻结第一人称、阻止外泄 | 维持低语义环境，移交猴群管理室 |
| **M5 档案与归属** | 确认其不是作者，只是工号与席位 | 区分"标注"与"解释"的责任边界 | 记录每次修订的责任来源 | 固定其为责任主体，保护公司不是作者 | 去姓名化、无署名化、封存责任 |
| **M6 状态确认组** | 判断其能否正常离开 | 测试其是否仍能区分打字和判断 | 测试其是否能否认个人意图 | 判定其是否还能作为人离开 | 触发猴子规则 |
| **M7 猴群管理室** | 不接触 | 预登记潜在对象 | 预登记 / 观察 | 接收不可释放对象 | 去人称化、保留输入能力 |

**非正式来源**（M8-M9 × 5 工种）：

| 来源 | 录入员 | 校对者 | 修改者 | 作者化个体 | 深度作者化 / 猴子化对象 |
|---|---|---|---|---|---|
| **M8 前员工残留** | 警告新人，但常因直说危险而反伤 | 鼓励其相信自己看到的差异 | 诱导或劝阻其修改，来源不稳定 | 让其写给后来者，可能加速作者化 | 退化成猴子规则碎片或敲击口令 |
| **M9 红领结文本** | 引起好奇，让其觉得自己"看懂了" | 夸奖其判断力 | 邀请其改得更好 | 诱导其承认"我是作者" | 试图让其恢复第一人称，以便重新绑定 |

#### 2.5.4 关键结构观察

- **正式部门 1-6 协同流水线**：M1 包装 + M2 提供材料 + M3 利用产能 + M4 维持安全 + M5 责任固定 + M6 判定流向 = 一条把"未受理文本"转化为"成立文本 + 可移交输入源"的工业流水线。M7 是流水线的输出端 sink。
- **M5 是公司核心保险机制**："保护公司不是作者"是整个 firewall stance（D14 v2 + D26 v2 + D30）的 administrative 实施。
- **非正式 vs 正式的非对称**：正式部门**不能**对后来者说话（仅处理 current employees）；非正式来源**只能**通过私下渠道触达——这正是它们"非正式"的定义。
- **对角线两端的镜像**：Tier 1 录入员 vs Tier 5 深度作者化 = 前者被全 7 部门 onboard；后者被 M2/M3 回收为输入源 + M4/M5/M7 处理为去人称资产。
- **MOKO ≠ 整个游戏**：battle scene 才是 MOKO（M4 安全部门旗下子工具）；shop / ritual / class picker / handbook / D29 等其他 scene 都**不是 MOKO**，是其他部门各自 surface delivery（详见 §2.5.5）。

#### 2.5.5 Scene → 主 Source 映射

| Scene | 主 Source | 备注 |
|---|---|---|
| **Battle 战斗界面** | **M4 安全部门 (MOKO)** | 字符层输入 / 节律 / boss modifier — MOKO 物化接口 |
| Shop / shop terminal | M1 人事与排班 + M2 外部文本回收 | 资源换购 + 词包/样本分发 |
| Ritual 附注台 | M3 文书实验部 | 附注流程 6 |
| Class picker | M1 人事与排班 | 包装层（deny→affirm flip 容器）|
| Handbook 守则 sticker | M5 档案与归属 | D31 layered (按 audience layer) |
| D29 状态确认 prompt | M6 状态确认组 | 5 项 × 4 退化阶段 |
| Endless 入口仪式 | M7 猴群管理 + M9 红领结 | D32 双 voice 同事件 |
| Cycle 末结算 / 评级 | M5 档案与归属 + M1 人事与排班 | 评级 + 调岗包装 |
| 主菜单 / loading | M1 + M5 | 工号 / 排班 / 历史档案 layered |
| 桌面便条 / scriptorNotes | **M8 前员工残留** | 非正式 voice 物理化 |

#### 2.5.6 棱镜效应示例

**事实**："员工 [工号] 在 Cycle X · BATCH 末完成基础录入考核，可承担更广职责"（即 Ch.2 transition · §044 deny→affirm flip）

玩家在 Tier 2 (校对者) 升职瞬间触达的 8 cells（M7 不接触此 transition）：

| Cell | 表述 |
|---|---|
| (M1, T2) | "员工 [工号] 岗位拓展。新职责见随附作业指引。" |
| (M2, T2) | "本批次差异样本已附 R2 标签。来源已遮蔽。" |
| (M3, T2) | "标注差异；不解释成因。" |
| (M4, T2) | "标注完毕后视线移至下一行。不复读。" |
| (M5, T2) | "标注归入流程归档；解释属个人意图，不予归档。" |
| (M6, T2) | "请区分'打字'和'写作'。" |
| (M8, T2) | "如果你觉得 X 不对，那它就是不对。" |
| (M9, T2) | "你的标注非常精准。" |

**8 cells × 同一升职事件** = 8 个不同来源同时描述同一事实，每个用自己的来源动机语调。玩家 cross-ref 才意识到说的是同一件事——这就是规则怪谈的棱镜效应。

#### 2.5.7 与旧"5 职业 × 三轨"矩阵的关系

旧矩阵 5 职业 × 三轨（公司 voice / 员工 voice / 异常 voice）= 新生成元矩阵的**粗粒度投射**：
- 旧"5 职业"行 = 新"5 工种"列（轴翻转）
- 旧"三轨"列 = 9 来源的粗分类：公司 voice ≈ M1-M7 / 员工 voice ≈ M8 / 异常 voice ≈ M9
- 新矩阵把 motive 来源细化到部门粒度（M1-M9），而不是模糊的"三轨"
- voice (V1-V7) 是 **form** 选择，不是矩阵 cell——每 cell 选择最合适的 voice form 表达其 motive

旧矩阵作为快速查询入口保留；**生成原理以新双轴矩阵为准**。
...
#### 5-tier vocab 分层铁律 🔒 LOCKED (2026-05-05 · post 5-tier calibration · **2026-05-08 部分修订：物种全名前移到 L1**)

每个职业 / endless 状态对应一个 surface vocab tier · L4 与 L5 之间是 **endless step-function gate**（不是 progressive disclosure）。**修订**：物种全名一列从 L5 前移到 L1（affix surface 直接显化）· step-function 在剩余 6 类 L5 reveal（lab vocab / 真名关联 / 食物经济 / 始祖揭示 / 玩家身份揭示）继续生效：

| Tier | 职业 / 状态 | 时机 | 词汇基调 | 灵长 frame | 关键词样例 |
|---|---|---|---|---|---|
| L1 | 录入员（`none`）| Cycle 1 早期 / 默认 | 街机 RPG + 物种学名 | **物种全名 YES · 其余 NO** | 黑帽松鼠猴 / 倭黑猩猩 / 智人 / 金币 / 倍率 / 💥 / 💰（物种全名作为 affix surface 直接显化 · 但**不**配 lab vocab / 真名关联 / 香蕉经济）|
| L2 | 校对者（待加 class）| Cycle 2-3 / 校对 class | 校对术语 + 物种学名 | **物种全名 YES · 其余 NO** | 勘误 / 对照 / 检视 / 批注 / 校核 |
| L3 | 修改者（`metamorph`）| Cycle 3-4 / 修改 class | 编辑术语 + 物种学名 | **物种全名 YES · 其余 NO** | 调整 / 重排 / 删改 / 校样 / 蜕化 |
| L4 | 作者（`wordsmith`）| Cycle 4-5 / 作者 class | 创作 / 署名 / 责任档案 + 物种学名 | **物种全名 YES · 其余 NO** | 创作 / 署名 / 发表 / 责任 / Subject N-1976（fictional anchor 编号 OK · 真名 NOT OK）|
| **L5** | **文本的一部分 / 猴子（endless）** | **Cycle 6+ endless** | **灵长 lab vocab + N-1976=Nim 关联 reveal + 食物经济 retcon** | **YES（全包）** | 食丸 / 代币 / chimpomat / WGTA / 香蕉 / Nim · Washoe · Lana 真名 / Subject XX-0001 始祖 / "你 = 智人" reveal |

##### 铁律

**base game (Cycle 1-5 · L1-L4 全 4 职业) 显化 vs 不显化** （2026-05-08 修订）：

**已允许 L1 起显化**：
- 物种全名（黑帽松鼠猴 / 智人 / 倭黑猩猩 等 · 作为 affix surface 名 · 详 §2.15.1 修订）

**仍永不显化**（L1-L4 全程 gate）：
- specific subject 真名关联（"N-1976 即 Nim Chimpsky"）
- primate research lab vocab（chimpomat / WGTA / Skinner-box / lexigram-keyboard / 食丸 / 代币）
- 香蕉 / 食物奖励经济（base game 用金币 · 1b507b1 保留）
- "你 = 智人" 玩家身份回指
- Subject XX-0001 始祖录入员 reveal

**L5 (endless) step-function 启用**（修订：剩余 6 类，不含物种全名）：
- 上述 5 类"仍永不显化"全部允许显化
- N-1976 与 Nim 真名关联仅此层 reveal（B8 reveal 后段落）
- "唯一逃脱机制 = 不玩 endless（保住猴子状态）" 已锁 § 第 5 职业 对位
- **物种全名不在此 gate 内**——L1 已显化，L5 段落直接 reference 即可（不再"reveal"）
...
具体物种（59 affix archive · 详 `generated/affix-l5-archives.mjs`）（2026-05-08 修订）：
- **L1 affix surface 直接显化物种全名**（黑帽松鼠猴 / 智人 等 · 玩家可见 affix 名 = species_zh）
- L4 archive_designation = 工号 / "前一代研究主体（项目档案已封存）"等 fictional化 · 用于责任档案 / 作者 lore（与 L1 affix surface 平行存在 · 不冲突：affix 名是该物种，archive_designation 是该 affix 在公司档案里的工号）
- L5 angle = lab vocab + Nim 真名关联仍 gate（物种全名本身不再 gate）· `generated/affix-l5-archives.mjs` 角色降级为 affix↔物种 lookup 表 + endless 段落 reference 资产
