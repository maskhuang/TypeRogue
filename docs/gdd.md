...
灵感来源于 **Balatro（小丑牌）** 的构筑深度与 **Backpack Battles** 的装备协同机制，游戏将打字这一核心输入转化为策略性的 Roguelike 体验。独特的 **键盘即棋盘** 系统让技能配置不仅关乎"选什么"，更关乎"放哪里"——QWERTY 键盘的 6 种位置关系（相邻/同行/同列/同手/同指/对称位）成为策略空间的核心维度。

86 个技能分为三种棋子角色（产出者/转化者/连接者），通过 5 种核心资源（基数/分数/倍率/时间/护盾）和 33 个附魔，在键盘上构建出独特的 Build。达成目标分数即可进入下一关，失败则重新开始——但每次失败都是学习新策略的机会。
...
**深度构筑:** 86 个技能（产出者/转化者/连接者）× 6 种键盘位置关系 × 33 个附魔 × 14 个遗物，提供数千种 Build 可能性，鼓励玩家探索"破格"组合和跨资源无限循环。
...
1. **🎹 键盘即棋盘** - QWERTY 布局的 6 种位置关系（相邻/同行/同列/同手/同指/对称位）成为策略空间，86 个技能 × 33 个附魔在键盘上构建阵型
...
2. **联动可理解性** - ✅ 已通过供给节奏教学解决：Act1 仅产出者 → Act2 引入转化者+连接者 → Act3 全开放，首次获得某类型时 tooltip 提示
...
| 角色 | 数量 | 定义 | 触发方式 |
|------|------|------|----------|
| **产出者** | 10（固定全进） | 直接生产资源 | 按键触发 |
| **转化者** | 40（每局随机20） | 读取源资源当前值，按系数产出目标资源（不消耗源） | 按键触发 |
| **连接者** | 36（每局随机18） | 复制效果 / 当资源增加时自动触发位置上的技能 | 复制=按键；资源触发=被动 |

每局技能池共 48 个（10+20+18）。
...
**连锁规则（三层防护）：**
1. **规则防 2 卡循环** — 资源触发类连接者跳过能产出被监听资源的技能
2. **允许 3+ 卡跨资源循环** — 终极 build 目标
3. **伪无限安全阀** — 循环中所有技能各以每秒 4 次自动触发，整关持续，玩家可同时打字

**附魔系统（33 个，Lv3 进化时抽 2 选 1）：**
- 只有产出者 + 转化者可进化（连接者固定 Lv1）
- 空间类 24 个（增幅/溅射/共鸣/排斥 × 6 位置）
- 变性型 5 个（额外产出另一种资源）
- 独立类 4 个（先驱/压轴/衰减/饥渴）

> 详细技能/附魔数据见 `docs/brainstorming-session-2026-03-03.md`

#### 分数系统

| 机制 | 描述 | 说明 |
|------|------|------|
| **基数** | 技能累积基数值 | 词语打完时参与算式结算 |
| **倍率** | 技能累积倍率值 | 乘数部分 |
| **词语结算** | 基数 × 倍率 = 本词得分 | 加入本关累计分 |
| **即时分数** | 分数类技能直接加总分 | 不经过算式 |
| **转化者** | 读源资源 × 系数产出目标 | 跨资源联动 |

#### 商店/构筑系统

| 机制 | 描述 |
|------|------|
| **关卡间商店** | 每关后可购买技能/遗物/升级/重绑定 |
| **金币获取** | 通关奖励 + 额外成就 |
| **技能升级** | Lv1→Lv2→Lv3 数值成长，Lv3 可附魔进化 |
| **遗物效果** | 被动加成，全局生效（14 个） |
| **Act 权重** | Act1: 产出者80% / Act2: 转化者50% / Act3: 连接者50% |
...
**技能升级（产出者+转化者，连接者固定 Lv1）:**

| 等级 | 获取方式 | 效果 |
|------|----------|------|
| Lv1 | 首次获得 | 基础效果 |
| Lv2 | 再次获得 | 产出者 ×1.6 / 转化者 k×1.5 |
| Lv3 | 第三次获得 | 产出者 ×2.4 / 转化者 k×2.0 + 可附魔进化 |

**附魔进化（Lv3 时触发）:**
- 从 33 个通用附魔池中随机抽 2 个，选 1 个附上
- 附魔为技能添加位置敏感的邻居效应（增幅/溅射/共鸣/排斥等）
...
**UI 布局（战斗中）:**
- 顶部 HUD：时间、倍率、金币
- 中央：词语显示区（主焦点）+ Balatro 式结算面板（基数×倍率=分数）
- 反馈：纯浮字 + 动画，5 种资源颜色区分（⚔️红, 🪙金, 🔥橙, ⏳蓝, 🛡️银）
- 无技能栏显示，连锁触发通过连续浮字表现节奏感
...
|------|------|------|
| 词库 | 5+ | JSON |
| 技能定义 | 86 + 33 附魔 | TypeScript/JSON |
| 遗物定义 | 14 | TypeScript/JSON |
...
| **E19** | 技能体系重构 | 86技能+33附魔+5资源+连锁规则 | 高 |
...
**E19: 技能体系重构**
- 5 种核心资源系统（基数/分数/倍率/时间/护盾）
- 产出者 10 个（含 Lv1-3 数值成长）
- 转化者 40 个（含系数 + 每局随机 20 进池）
- 连接者 36 个（复制类 6 + 资源触发类 30）
- 附魔系统 33 个（空间 24 + 变性 5 + 独立 4）
- 三层连锁规则 + 伪无限安全阀
- 遗物清理（删 4 改 1 留 14）
- 战斗 UI 改造（移除技能栏，纯浮字反馈，5 资源颜色）
- 商店 Act 权重 + 新手引导节奏
