...
| Kick 脉冲 | 0-0.007 | 35-60Hz triangle | BGM 柔化节奏 |
| **Pad 层** | **0.018-0.028** | **65Hz triangle ×3** | **BGM 温暖基底** |
| **噪声氛围** | **0.004-0.014** | **300-1200Hz bandpass** | **空间感填充** |

### 张力参数表（滤波器驱动，无振荡器创建/销毁）

| Level | 低通截止 | Pad音量 | 噪声音量 | 噪声中心频 | Osc3频率 | 渐变时间 | 特殊 |
|-------|---------|---------|---------|-----------|---------|---------|------|
| 0/1 | 400Hz | 0.018 | 0.004 | 300Hz | 130.81Hz(C3) | 800ms | - |
| 2 | 600Hz | 0.022 | 0.007 | 500Hz | 130.81Hz | 800ms | - |
| 3 | 900Hz | 0.025 | 0.010 | 800Hz | 261.63Hz(C4) | 600ms | - |
| 4 | 1200Hz | 0.028 | 0.014 | 1200Hz | 261.63Hz | 400ms | 6Hz噪声颤音 |

Release: 200ms 快速恢复基线。

### 张力传达策略

用滤波器参数变化传达张力，而非创建/销毁不协和振荡器：
- 低通截止升高 → 更亮更尖锐 → 紧张感
- 噪声音量+中心频率升高 → 更嘈杂 → 不安感
- Osc3 从 C3→C4 → 更高音域 → 压迫感
- Level 4 的 6Hz 噪声颤音 → 闪烁效果 → 危急感
- 通关释放 200ms 快速恢复基线 → "呼吸感"释放
