...
const ZH: Record<string, string> = {
  ...
  'affix_desc.relay': '自身不产出；指定关系的匹配技能触发时叠层，满层触发1个匹配技能',
  ...
  'affix_desc.war_drum': '自身不产出；触发叠层，指定关系的匹配技能+暴击率',
  'affix_desc.phase_shift': '读取{source}的产出当温度(T1={t1},T2={t2})，跨阈值时产出跳升；高温消耗资源',
  'affix_desc.endo_exo': '读取{source}的产出，高于阈值{threshold}时高产出+消耗，低于时低产出',
  'affix_desc.fusion': '读取{sourceA}+{sourceB}的产出，同时达到阈值时高倍产出+双消耗，否则惩罚',
  ...
  'affix_desc.parity': '奇数层+产出，偶数层+{evenK}暴击率',
  ...
  'affix_desc.leverage': '读取{source}，本词产出超过阈值{threshold}时加成，低于时扣减',
  'affix_desc.option': '读取{source}，本词产出超过阈值{threshold}时按超出量加成，未达时扣减',
  'affix_desc.hedge': '{sourceA}和{sourceB}的产出越接近，加成越高',
  'affix_desc.burst': '连续暴击次数越多，暴击倍率越高；未暴击时连击归零',
  'affix_desc.zero_in': '连续未暴击次数越多，下次暴击的倍率越高',
  'affix_desc.sharpshooter': '暴击率越低，暴击时的倍率加成越高',
  'affix_desc.bridge': '相邻邻居之间的唯一连接点时，大额加成',
...
const EN: Record<string, string> = {
  ...
  'affix_desc.pulse': 'Stacks on each trigger; self-triggers when full',
  ...
  'affix_desc.resonance': 'Stacks when matched skills in range trigger; self-triggers when full',
  'affix_desc.mirror': 'Copies a random affix from a skill in range at stage end; replaces itself next stage',
  'affix_desc.splash': 'Produces no output; triggers 1 matched skill in range when stacks full',
  ...
  'affix_desc.relay': 'Produces no output; stacks when matched skills trigger, triggers 1 matched skill when full',
  ...
  'affix_desc.war_drum': 'Produces no output; stacks on trigger, matched skills in range gain crit rate',
  'affix_desc.phase_shift': 'Reads {source} as temperature (T1={t1},T2={t2}); jumps at thresholds; high temp consumes',
  'affix_desc.endo_exo': 'Reads {source}; above threshold {threshold} = high output + consume; below = low',
  'affix_desc.fusion': 'Reads {sourceA}+{sourceB}; both above threshold = high output + dual consume; else penalty',
  'affix_desc.flow': 'Same-resource neighbors with higher output boost yours by the difference',
  'affix_desc.confluence': 'More diverse resource types among neighbors = higher bonus',
  'affix_desc.turbulence': 'Greater strength variance among neighbors = higher bonus',
  ...
  'affix_desc.parity': 'Odd stacks: +output; Even stacks: +{evenK} crit chance',
  'affix_desc.prime': 'At prime stacks (2,3,5,7,11…): big output bonus; composites: nothing',
  'affix_desc.match': 'More pairs of neighbors sharing same stack count = more output',
  ...
  'affix_desc.leverage': 'Reads {source}; word output above {threshold} = bonus; below = penalty',
  'affix_desc.option': 'Reads {source}; word output above {threshold} = scaled bonus; below = penalty/trigger',
  'affix_desc.hedge': '{sourceA} and {sourceB} output closer together = more bonus',
  ...
  'affix_desc.bridge': 'Big bonus when this key is the only link between adjacent neighbors',
  'affix_desc.clique': 'Larger group of mutually connected neighbors = more output',
