...
const ZH: Record<string, string> = {
  ...
  'affix_desc.pulse': '每次触发叠层，每叠N层立刻自触发一次',
  'affix_desc.crit': '触发时有概率暴击',
  'affix_desc.cascade': '上一个按键与当前键满足指定位置关系时，产出倍增',
  'affix_desc.void': '指定关系的空位越多加成越高',
  'affix_desc.resonance': '指定关系的匹配技能触发时叠层，每叠N层自触发一次',
  'affix_desc.mirror': '每关结束时从指定关系的邻居中随机复制一个词条，下关替代自身生效',
  'affix_desc.splash': '自身不产出；每叠N层触发指定关系的1个匹配技能',
  'affix_desc.amplify': '自身不产出；触发叠层，指定关系的匹配技能产出+自身基础值',
  'affix_desc.conduit': '自身不产出，指定关系的匹配技能触发时额外触发一次',
  'affix_desc.relay': '自身不产出；指定关系的匹配技能触发时叠层，每叠N层触发1个匹配技能',
  ...
  'affix_desc.leverage': '本词产出超过阈值时加成，低于时扣减',
  'affix_desc.option': '本词产出超过阈值时按超出量加成，未达时每次触发扣减固定量',
  'affix_desc.hedge': '两种资源的产出越接近，加成越高',
  'affix_desc.burst': '连续暴击次数越多，暴击倍率越高；未暴击时连击归零',
  'affix_desc.zero_in': '连续未暴击次数越多，下次暴击的倍率越高',
  'affix_desc.sharpshooter': '暴击率越低，暴击时的倍率加成越高',
  'affix_desc.bridge': '指定关系的邻居之间的唯一连接点时，大额加成',
  'affix_desc.clique': '自身与指定关系的邻居中两两相连的最大组越大，产出越高',
  'affix_desc.component': '沿相邻关系连成一片的技能数量越多，产出越高',
  'affix_desc.decorator': '放大同技能其他词条的产出加成',
  'affix_desc.reflect': '技能词条越多、等级越高，产出越高',
  'affix_desc.monkey_patch': '每关随机修改同技能一个词条的效果倍率',
  ...
  'param.charge': '1秒蓄满 暴击率上限{max}%',
  'param.decay': '初始暴击率{init}% 衰减{decay}% 下限{floor}%',
  'param.pulse': '每{interval}层自触发',
  'param.crit': '{chance}% 暴击率',
  'param.void': '{rel}每空位+{pct}%',
  'param.resonance': '{rel}每{n}层自触发1次',
  'param.amplify': '{rel}叠层+匹配产出加成',
  'param.cascade_fallback': '上键关系',
  'param.outcast': '+{pct}%',
  'param.gravity': '概率×{mult}',
  'param.recurse': '+{pct}%暴击率 暴击重触发',
  'param.taboo': '+{pct}%暴击率 / 未暴击负产出',
  'param.fallacy': '每次未暴击 +{k}%暴击率',
  'param.rainbow': '随机资源',
  'param.mirror': '{rel}镜像复制',
  'param.splash': '{rel}每{n}层触发1个匹配技能',
  'param.relay': '{rel}每{n}层触发1个匹配技能',
  'param.war_drum': '{rel}每层+{pct}%暴击率',
  'param.ligature': '连字×{pct}%',
  'param.twin': '双附魔',
  'param.parity': '奇+{odd}%产出 / 偶+{even}%暴击',
  'param.prime': '素数时+{k}%×叠层数',
  'param.match': '{rel} +{k}%/配对',
  'param.entropy': '+{k}%×信息熵',
  'param.cipher': '+{k}%×字母跳跃',
  'param.pattern': '+{k}%×模式稀有度',
  'param.leverage': '读{icon}:阈值{threshold} +{k}%×超出量',
  'param.option': '读{icon}:阈值{threshold} 未达-{k}%/次',
  'param.hedge': '读{iconA}+{iconB}:接近+{k}%',
  'param.burst': '连击+{k}%暴击倍率/层',
  'param.zero_in': 'miss补偿+{k}%暴击倍率/层',
  'param.sharpshooter': '低暴击率+{k}%暴击倍率',
  'param.bridge': '相邻 桥+{k}%',
  'param.clique': '{rel} 团+{k}%/成员',
  'param.component': '连通+{k}%/成员',
  'param.decorator': '放大+{k}%',
  'param.reflect': '+{k}%×(词条数×等级)',
  'param.monkey_patch': '随机×{low}~{high}',
  'param.multiply': '×{val}',
  'param.cluster': '+{k}%/辅音段',
  'param.coverage': '+{k}%/字母种类',
  'param.bigram': '+{k}%×罕见度',
  'param.flow': '{rel} +{k}%/落差',
  'param.confluence': '{rel} +{k}%×多样性',
  'param.turbulence': '{rel} +{k}%×极差',
  'param.phase_shift': '读{icon}:T1={t1},T2={t2}',
  'param.endo_exo': '读{icon}:阈值{threshold}',
  'param.fusion': '{iconA}+{iconB}点火',
  'param.innate': '开局自动触发{n}次',
  'param.counter': '{rel} 充能{n}次',
  'param.exhaust': '×{mult} 共{n}次',
  'param.ethereal': '词条+1级 限1关',
...
const EN: Record<string, string> = {
  ...
  'affix_desc.pulse': 'Stacks on each trigger; self-triggers once every N stacks',
  ...
  'affix_desc.resonance': 'Stacks when matched skills in range trigger; self-triggers once every N stacks',
  'affix_desc.mirror': 'Copies a random affix from a skill in range at stage end; replaces itself next stage',
  'affix_desc.splash': 'Produces no output; triggers 1 matched skill in range every N stacks',
  'affix_desc.amplify': 'Produces no output; self-stacks on trigger, matched skills in range gain base value bonus',
  'affix_desc.conduit': 'Produces no output; matched skills in range get +1 trigger',
  'affix_desc.relay': 'Produces no output; stacks when matched skills trigger, relays to 1 matched skill every N stacks',
  ...
  'affix_desc.counter': 'When self or matched-relation skill suffers negative affix effects, spend charge to cancel loss (recharges each stage)',
  ...
  'affix_desc.leverage': 'Word output above threshold = bonus; below = penalty',
  'affix_desc.option': 'Word output above threshold = scaled bonus; below = fixed penalty per trigger',
  'affix_desc.hedge': 'Two resources\' output closer together = more output',
  ...
  'affix_desc.bridge': 'Big bonus when this key is the only link between matched neighbors',
  'affix_desc.clique': 'Larger group of mutually connected neighbors (all pairs linked) = more output',
  ...
  'affix_desc.monkey_patch': 'Each stage, randomly modifies one affix\'s effect multiplier',
  ...
  'param.charge': '1s full charge, crit cap {max}%',
  ...
  'param.resonance': '{rel} self-trigger every {n} stacks',
  ...
  'param.fallacy': '+{k}% crit per non-crit',
  'param.rainbow': 'random resource',
  'param.mirror': '{rel} mirror copy',
  'param.splash': '{rel} triggers 1 matched skill every {n} stacks',
  'param.relay': '{rel} relays to 1 matched skill every {n} stacks',
  'param.war_drum': '{rel} +{pct}% crit/stack',
  'param.ligature': 'ligature ×{pct}%',
  'param.twin': 'dual enchant',
  'param.parity': 'odd +{odd}% output / even +{even}% crit',
  'param.prime': 'prime +{k}% × stacks',
  'param.match': '{rel} +{k}%/pair',
  'param.entropy': '+{k}% × entropy',
  'param.cipher': '+{k}% × letter jump',
  'param.pattern': '+{k}% × pattern rarity',
  'param.leverage': 'read {icon}: threshold {threshold} +{k}% × excess',
  'param.option': 'read {icon}: threshold {threshold} miss -{k}%/time',
  'param.hedge': 'read {iconA}+{iconB}: close +{k}%',
  'param.burst': 'streak +{k}% crit mult/stack',
  'param.zero_in': 'miss +{k}% crit mult/stack',
  'param.sharpshooter': 'low crit +{k}% crit mult',
  'param.bridge': 'adjacent bridge +{k}%',
  'param.clique': '{rel} clique +{k}%/member',
  'param.component': 'connected +{k}%/member',
  'param.decorator': 'amplify +{k}%',
  'param.reflect': '+{k}% × (affixes × level)',
  'param.monkey_patch': 'random ×{low}~{high}',
  'param.multiply': '×{val}',
  'param.cluster': '+{k}%/consonant cluster',
  'param.coverage': '+{k}%/letter type',
  'param.bigram': '+{k}% × rarity',
  'param.flow': '{rel} +{k}%/gap',
  'param.confluence': '{rel} +{k}% × diversity',
  'param.turbulence': '{rel} +{k}% × range',
  'param.phase_shift': 'read {icon}: T1={t1}, T2={t2}',
  'param.endo_exo': 'read {icon}: threshold {threshold}',
  'param.fusion': '{iconA}+{iconB} ignite',
  'param.innate': 'auto-trigger {n}× at start',
  'param.counter': '{rel} {n} charges',
  'param.exhaust': '×{mult} {n} uses',
  'param.ethereal': '+1 affix level, 1 stage only',
  ...
  'est.turbulence': 'Turbulence +{pct}%',
  'est.turbulence_detail': '(range {pct}%)',
  'est.cluster': 'Cluster +{pct}%',
  'est.coverage': 'Coverage +{pct}%',
  'est.bigram': 'Bigram +{pct}%',
  'est.entropy_est': 'Entropy +{pct}%',
  'est.cipher': 'Cipher +{pct}%',
  'est.pattern': 'Pattern +{pct}%',
  'est.word_avg_detail': '({n} words avg)',
  'est.bridge': 'Bridge +{pct}%',
  'est.bridge_no': 'Bridge (not bridge)',
  'est.clique': 'Clique +{pct}%',
  'est.clique_detail': '({n}-clique)',
  'est.component': 'Component +{pct}%',
