...
export interface TriggerContext {
  ...
  /** 发出极速的源 skillId（on_haste_emitted 用 · scope 匹配在 hook 层做 · relic 源时为 undefined → 不触发）*/
...
export function evaluateTrigger(spec: TriggerSpec, ctx: TriggerContext): boolean {
  switch (spec.type) {
    ...
    case 'on_haste_emitted':
      // hook 层已保证 scope 匹配（源 skill 在 scope 内）；evaluator 只校验 context 完整
      return ctx.emitterSkillId !== undefined
