...
// === Task 6: independent 附魔修饰叠层 ===

describe('triggerAmplifier — independent 附魔', () => {
  beforeEach(() => { resetState() })

  it('pioneer: 本词首次叠层 ×2', async () => {
    const { triggerAmplifier } = await import('../../../src/systems/skills')
    state.player.bindings.set('a', 'amp_base_add_adjacent')
    state.player.skills.set('amp_base_add_adjacent', { level: 1 })
    state.player.enchantedSkills.set('amp_base_add_adjacent', 'ench_pioneer')
    state.amplifierStacks.set('amp_base_add_adjacent', 0)

    synergy.wordSkillCount = 0 // 本词首次
    triggerAmplifier('amp_base_add_adjacent', 'a')
    // enchMult = 2.0, ceil(1 * 2.0) = 2
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(2)
  })

  it('pioneer: 非首次叠层 +1', async () => {
    const { triggerAmplifier } = await import('../../../src/systems/skills')
    state.player.bindings.set('a', 'amp_base_add_adjacent')
    state.player.skills.set('amp_base_add_adjacent', { level: 1 })
    state.player.enchantedSkills.set('amp_base_add_adjacent', 'ench_pioneer')
    ...
    synergy.wordSkillCount = 1 // 非首次
    triggerAmplifier('amp_base_add_adjacent', 'a')
    // enchMult = 1.0, ceil(1 * 1.0) = 1
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(1)
  })

  it('finale: 本词最后一个绑定键叠层 ×3', async () => {
    const { triggerAmplifier } = await import('../../../src/systems/skills')
    state.player.bindings.set('a', 'amp_base_add_adjacent')
    state.player.skills.set('amp_base_add_adjacent', { level: 1 })
    state.player.enchantedSkills.set('amp_base_add_adjacent', 'ench_finale')
    state.amplifierStacks.set('amp_base_add_adjacent', 0)

    // 模拟当前词 "cat"，'a' 是最后一个绑定键（index=1）
    // 只有 'a' 被绑定，c 和 t 未绑定
    state.player.word = 'cat'
    state.player.index = 1 // 'a' 的位置

    triggerAmplifier('amp_base_add_adjacent', 'a')
    // enchMult = 3.0 (ench_finale effectValue), ceil(3.0) = 3
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(3)
  })

  it('finale: 非最后一个绑定键叠层 +1', async () => {
    const { triggerAmplifier } = await import('../../../src/systems/skills')
    state.player.bindings.set('a', 'amp_base_add_adjacent')
    state.player.bindings.set('s', 'prod_burst') // s 也绑定了
    state.player.skills.set('amp_base_add_adjacent', { level: 1 })
    state.player.enchantedSkills.set('amp_base_add_adjacent', 'ench_finale')
    state.amplifierStacks.set('amp_base_add_adjacent', 0)

    // 词 "fast"，'a' 在 index=1，但 's' 在 index=2 也绑定了 → 'a' 不是最后一个绑定键
    state.player.word = 'fast'
    state.player.index = 1

    triggerAmplifier('amp_base_add_adjacent', 'a')
    // enchMult = 1.0 (非最后绑定键), ceil(1.0) = 1
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(1)
  })

  it('decay: 首次 ×2.5, 后续衰减', async () => {
    const { triggerAmplifier } = await import('../../../src/systems/skills')
    state.player.bindings.set('a', 'amp_base_add_adjacent')
    state.player.skills.set('amp_base_add_adjacent', { level: 1 })
    state.player.enchantedSkills.set('amp_base_add_adjacent', 'ench_decay')
    state.amplifierStacks.set('amp_base_add_adjacent', 0)

    // 第1次: enchMult = 2.5 * 0.7^0 = 2.5, ceil(2.5) = 3
    triggerAmplifier('amp_base_add_adjacent', 'a')
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(3)

    // 第2次: enchMult = 2.5 * 0.7^1 = 1.75, ceil(1.75) = 2
    triggerAmplifier('amp_base_add_adjacent', 'a')
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(5) // 3 + 2

    // 第3次: enchMult = 2.5 * 0.7^2 = 1.225, ceil(1.225) = 2
    triggerAmplifier('amp_base_add_adjacent', 'a')
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(7) // 5 + 2
  })
})

// === Task 6.3: ench_thirst 增幅者资源获取 ===

describe('getEnchantmentMultiplier — ench_thirst 增幅者支持', () => {
  beforeEach(() => { resetState() })

  it('ench_thirst: 低资源时增幅者叠层增量提高', async () => {
    const { triggerAmplifier } = await import('../../../src/systems/skills')
    // base 增幅者 + thirst 附魔
    state.player.bindings.set('a', 'amp_base_add_adjacent')
    state.player.skills.set('amp_base_add_adjacent', { level: 1 })
    state.player.enchantedSkills.set('amp_base_add_adjacent', 'ench_thirst')
    state.amplifierStacks.set('amp_base_add_adjacent', 0)

    // 低分数（base 资源映射到 score/targetScore ratio）
    state.score = 0
    state.targetScore = 100
    // ratio = 0/100 = 0, thirst = 1 + 2*(1-0) = 3.0
    triggerAmplifier('amp_base_add_adjacent', 'a')
    // ceil(1 * 3.0) = 3
    expect(state.amplifierStacks.get('amp_base_add_adjacent')).toBe(3)
  })
