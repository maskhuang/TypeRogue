...
describe('AffixType', () => {
  ...
  it('should have exactly 54 values', () => {
    expect(allAffixTypes).toHaveLength(54)
  })
  ...
  it('should contain all expected types', () => {
    const expected = [
      ...
      'entropy', 'cipher', 'pattern', 'leverage', 'option', 'hedge', 'burst', 'zero_in', 'sharpshooter',
      'bridge', 'clique', 'component', 'decorator', 'reflect', 'monkey_patch',
...
describe('AFFIX_CATEGORY_MAP', () => {
  ...
  it('should have 6 numeric, 6 crit, 7 stack, 6 topology, 6 word_sense, 6 meta_rule', () => {
    ...
    expect(counts.crit).toBe(9)
    expect(counts.stack).toBe(9)
    expect(counts.topology).toBe(9)
    expect(counts.word_sense).toBe(9)
    expect(counts.meta_rule).toBe(9)
  })
})
